            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', fontSize: '12px', marginBottom: '16px' }}>
              ...
              {legs.length === 1 ? (
                ...
                <div>
                  ...
                  <div style={{ color: '#718096', marginBottom: '4px' }}>{fmtDate(legs[0].checkin)} to {fmtDate(legs[legs.length - 1].checkout)}</div>
                  {legs.map((l, i) => (
                    <div key={l.id} style={{ marginBottom: '3px' }}>
                      <span style={{ fontWeight: 600 }}>{`${i + 1}. ${l.hotel.name}`}</span>
                      <span style={{ color: '#718096' }}>{` · ${l.hotel.location} · ${fmtDate(l.checkin)} to ${fmtDate(l.checkout)} · ${l.rooms} room${l.rooms > 1 ? 's' : ''}`}</span>
                    </div>
                  ))}
                </div>
              )}
